...
// 初期化関数
function initResult(): Record<string, { win: number; lose: number; draw: number }> {
  const result: Record<string, { win: number; lose: number; draw: number }> = {};
  teams.forEach(team => {
...
async function updateResults() {
  ...
  const processed = loadJson<string[]>(PROCESSED_PATH, []);

  const scheduleRes = await fetch(scheduleUrl);
  const scheduleData = await scheduleRes.json() as any;
  const gamePks: string[] = scheduleData.games.map((g: any) => g.gamePk);

  for (const gamePk of gamePks) {
    if (processed.includes(gamePk)) continue;
